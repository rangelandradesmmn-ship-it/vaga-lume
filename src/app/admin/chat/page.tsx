...
export default function AdminChatPage() {
  ...
  async function initGuardians() {
    ...
    // Buscar todos os pais cadastrados
    const { data: guardiansData } = await supabase
      ...
      .select(`
        ...
        user_id,
        child_id,
        profiles!guardians_user_id_fkey (name),
        children!guardians_child_id_fkey (name)
      `);

    const parsedGuardians = (guardiansData || []).map(g => ({
      id: g.id,
      user_id: g.user_id,
      child_id: g.child_id,
      name: Array.isArray(g.profiles) ? g.profiles[0]?.name : g.profiles?.name,
      sub_name: Array.isArray(g.children) ? g.children[0]?.name : g.children?.name,
      type: 'GUARDIAN'
    }));
    ...
    const parsedProfs = (profsData || []).map(p => ({
      ...
      child_id: null,
      name: p.name,
      sub_name: (Array.isArray(p.professionals) ? p.professionals[0]?.role_title : p.professionals?.role_title) || 'Profissional',
      type: 'PROFESSIONAL'
    }));

    const combinedList = [...parsedGuardians, ...parsedProfs];

    // Buscar chats para ver mensagens não lidas
    const { data: allChats } = await supabase
      .from('chats')
      .select('id, guardian_id, child_id');
      ...
      if (latestMessages) {
        allChats.forEach(chat => {
          ...
          if (chatMsgs.length > 0) {
            ...
            if (user && lastMsg.sender_id !== user.id) {
              ...
              if (msgTime > readTime) {
                unreadMap[`${chat.guardian_id}_${chat.child_id}`] = true;
              }
            }
          }
        });
    ...
    const enriched = combinedList.map(g => ({
      ...g,
      hasUnread: unreadMap[`${g.user_id}_${g.child_id}`] || false
    }));

    // Ordenar: Primeiro os com não lidas, depois alfabeticamente
  ...
  const handleSelectChat = async (guardian: any) => {
    // Buscar se já existe um chat
    let query = supabase
      .from('chats')
      .select('id')
      .eq('guardian_id', guardian.user_id)
      .is('recipient_id', null);
      
    if (guardian.child_id) {
      query = query.eq('child_id', guardian.child_id);
    } else {
    ...
    if (!chatData) {
      ...
      const { data: newChat } = await supabase
        .from('chats')
        .insert({ guardian_id: guardian.user_id, child_id: guardian.child_id || null, recipient_id: null })
        .select('id')
        .limit(1);
      chatData = newChat?.[0];
    }
    ...
    if (chatData) {
      ...
      // Limpa do estado atual para a bolinha sumir na hora
      setGuardiansList(prev => prev.map(g => g.id === guardian.id ? { ...g, hasUnread: false } : g));
      
      setSelectedChat({ ...guardian, chat_id: chatData.id });
